(function () {
    ...
    $clickElem.addEventListener('click', function () {
        ...
        var onDOMContentLoaded = function () {
            ...
            filterL.frequency.value = 500;
            filterM.frequency.value = 1000;
            filterH.frequency.value = 2000;
            ...
            document.getElementById('button-filter-low').addEventListener('click',
                function volumeL() {
                    ...
                    analyser.getByteFrequencyData(dataArray);
                    var sumL1 = dataArray.slice(0, 25).reduce(function (a, x) {
                        return a + x;
                    });
                    var averageL1 = sumL1 / 24;
                    ...
                    var systemL = (averageL2 * 100) - 40;

                    if ((systemL >= -40) && (systemL <= 20)) {
                        filterL.gain.value = systemL;
                        document.getElementById('output-filter-low').textContent = systemL;
                    } else if (20 < systemL) {
                        filterL.gain.value = 20;
                        document.getElementById("output-filter-low").textContent = 20;
                    } else {
                        filterL.gain.value = -40;
                        document.getElementById("output-filter-low").textContent = -40;
                    }
                }, false);

            // Control EqualizerM
            var numberM = Array(100).fill(0);
            document.getElementById('button-filter-middle').addEventListener('click',
                function volumeM() {
                    requestAnimationFrame(volumeM);
                    analyser.getByteFrequencyData(dataArray);
                    var sumM1 = dataArray.slice(24, 93).reduce(function (a, x) {
                        return a + x;
                    });
                    var averageM1 = sumM1 / 69;
                    ...
                    var systemM = (averageM2 * 100) - 40;

                    if ((systemM >= -40) && (systemM <= 20)) {
                        filterM.gain.value = systemM;
                        document.getElementById('output-filter-middle').textContent = systemM;
                    } else if (20 < systemM) {
                        filterM.gain.value = 20;
                        document.getElementById("output-filter-middle").textContent = 20;
                    } else {
                        filterM.gain.value = -40;
                        document.getElementById("output-filter-middle").textContent = -40;
                    }
                }, false);
            ...
            document.getElementById('button-filter-high').addEventListener('click',
                function volumeH() {
                    ...
                    analyser.getByteFrequencyData(dataArray);
                    var sumH1 = dataArray.slice(92, 1023).reduce(function (a, x) {
                        return a + x;
                    });
                    var averageH1 = sumH1 / 931;
                    ...
                    var systemH = (averageH2 * 100) - 40;
                    console.log(systemH);

                    if ((systemH >= -40) && (systemH <= 20)) {
                        filterH.gain.value = systemH;
                        document.getElementById('output-filter-high').textContent = systemH;
                    } else if (20 < systemH) {
                        filterH.gain.value = 20;
                        document.getElementById("output-filter-high").textContent = 20;
                    } else {
                        filterH.gain.value = -40;
                        document.getElementById("output-filter-high").textContent = -40;
                    }
                }, false);
